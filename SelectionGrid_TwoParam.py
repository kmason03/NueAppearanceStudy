...
def main():
    ...
    sin22theta_list= sin22theta_list*.0001
    ...
    for m in np.arange(ntestbins):
        ...
        for s in np.arange(ntestbins):
            ...
            rwt_sys_m,det_sys_m = getSysErrors(numbins,varName,bdtmode,0.95)
            det_sys_m = getDetSysTot(det_sys_m,pred_e,pred_mu)
            cov += det_sys_m * np.outer(pred,pred)
            # write detvar to a text file
            if(m==0 and s==0):
                file = open("fracdetvar.txt","w")
